...
- **Mission economies.** Instead of letting agents trade opportunistically, define a concrete shared objective — map a protein family, optimize a city's energy grid — along with a budget and success metrics, and let agents self-organize around the task. The paper argues that this is more trahen the goal is specific and bounded, rather than something as diffuse as "human flourishing."
- **Trust & safety infrastructure.** There needs to be a way to trace who deployed an agent and what it did. Agents should have to pass safety checks before they're allowed to participate, and reputation scores help decide who to interact with. And for when things break anyway: circuit breakers and kill switches.
